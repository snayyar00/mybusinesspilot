...
const NavLogo = () => <img className='h-8 w-58' src={logo} alt='BusinessPilot' />;
...
export default function AppNavBar() {
  ...
  return (
    <header className='absolute inset-x-0 top-0 z-50 shadow sticky bg-gray-300 bg-opacity-50 backdrop-blur-lg backdrop-filter dark:border dark:border-gray-100/10 dark:bg-boxdark-2'>
      <nav className='flex items-center justify-between p-6 lg:px-8' aria-label='Global'>
        <div className='flex lg:flex-1'>
          <a href='/' className='-m-1.5 p-1.5'>
            <img className='h-8 w-58' src={logo} alt='BusinessPilot' />
          </a>
        </div>
      ...
      <Dialog as='div' className='lg:hidden' open={mobileMenuOpen} onClose={setMobileMenuOpen}>
        ...
        <Dialog.Panel className='fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white dark:text-white dark:bg-boxdark px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10'>
          <div className='flex items-center justify-between'>
            <a href='/' className='-m-1.5 p-1.5'>
              <span className='sr-only'>BusinessPilot</span>
              <NavLogo />
            </a>
            <button
              type='button'
